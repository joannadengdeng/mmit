"""
Freeze Tuning 真实模型实验 — Google Colab 版
=============================================
在 Colab 免费 T4 GPU 上用 Qwen2-VL-2B 跑完整的 Freeze Tuning 流程。
每一步都详细打印内部状态。
...
# ══════════════════════════════════════════════════
# Cell 1: 安装依赖（在 Colab 里运行）
# ══════════════════════════════════════════════════

CELL_1 = """
!pip install -q transformers accelerate torch Pillow qwen-vl-utils
"""

# ══════════════════════════════════════════════════
# Cell 2: 完整实验代码（在 Colab 里运行）
# ══════════════════════════════════════════════════
...
冻结参数:       {frozen_final:,} ({frozen_final/total_params*100:.1f}%)
可训练参数:     {trainable_final:,} ({trainable_final/total_params*100:.1f}%)
训练策略:       冻结全部 → 解冻最后 {num_to_unfreeze} 层

训练 12 步:     loss {losses[0]:.4f} → {losses[-1]:.4f}
Checkpoint:     {file_size/1024/1024:.1f} MB (全量保存需 ~{total_params*2/1024/1024:.0f} MB)

对应 freeze.py 的代码:
  _prepare_model_impl() → Step 3-4 (冻结 + 解冻)
  compute_loss()        → Step 7 (CE loss)
  get_trainable_params()→ Step 5 (传给 optimizer)
  save_checkpoint()     → Step 12 (只保存可训练参数)
  load_for_inference()  → Step 13 (加载基础模型 + 覆盖训练过的参数)
""")
"""
...
2. 菜单 → 运行时 → 更改运行时类型 → T4 GPU
3. 新建一个代码单元格，粘贴 Cell 1 的内容（安装依赖）：
""")
print(CELL_1)
print("""
4. 再新建一个代码单元格，粘贴 Cell 2 的全部内容（实验代码）
5. 运行！

Cell 2 的内容已保存为字符串，你也可以直接复制
exercises/colab_freeze_experiment.py 里 CELL_2 变量的内容。
""")

# 也把 cell 2 写到单独文件方便复制
output_path = os.path.join(os.path.dirname(__file__), "colab_cell2.py")
with open(output_path, "w") as f:
    f.write(CELL_2.strip())
print(f"Cell 2 代码已单独保存到: {output_path}")
